...
export function VideoGallery({ videos }: VideoGalleryProps) {
  ...
  const handleTap = useCallback(
    ...
    [activeIndex, activate, deactivate]
  );
      ...
      <div className="flex md:hidden flex-col gap-3 w-full">
        {videos.map((video, idx) => {
          ...
          return (
            <motion.div
              key={video.src}
              className="relative cursor-pointer overflow-hidden rounded-lg border border-border-cool"
              animate={{ height: isActive ? 360 : 100 }}
            ...
            >
              ...
              {!isActive && (
                <div className="absolute right-4 top-1/2 -translate-y-1/2">
                  <span className="font-body text-[10px] tracking-[0.15em] uppercase text-cream/50">
                    Tap to play
                  </span>
                </div>
              )}
